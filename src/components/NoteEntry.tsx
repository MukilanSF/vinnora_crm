...
import { Note, Lead, Customer, Deal } from '../utils/types';
...
const NoteEntry: React.FC<NoteEntryProps> = ({ isOpen, onClose, leads, customers, deals, onSave }) => {
  const [selectedEntity, setSelectedEntity] = useState('');
  const [selectedEntityType, setSelectedEntityType] = useState<'lead' | 'customer' | 'deal'>('lead');
  ...
  const handleSave = async () => {
    if (!selectedEntity || !content.trim()) return;
  ...
  const getEntityOptions = () => {
    switch (selectedEntityType) {
      ...
      case 'deal':
        return deals.map(deal => ({ id: deal.id, name: deal.dealName }));
      default:
        return [];
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/80 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-800 rounded-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {(['lead', 'customer', 'deal'] as const).map((entityType) => (
                <button
                  key={entityType}
                  onClick={() => {
        ...
        <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 dark:border-gray-800">
          ...
          <button
            onClick={handleSave}
            disabled={!selectedEntity || !content.trim() || isSaving}
            className={`flex items-center space-x-2 px-6 py-2 rounded-lg font-medium transition-all ${
              !selectedEntity || !content.trim() || isSaving
